...
const formatKES = (v) =>
  typeof v === "number" && v > 0 ? `KES ${v.toLocaleString("en-KE")}` : "";
...
export default function UniversalEnrollmentForm({ defaultProgramType = "", onClose }) {
  ...
  useEffect(() => {
    ...
    setForm((f) => ({
      ...f,
      country: f.country || firstCountry.name,
      code: f.code || firstCountry.code,
      nationality: f.nationality || firstCountry.nationality,
    }));
  }, [firstCountry?.name]);
  ...
  const handleChange = (e) => {
    ...
    if (name === "interest") {
      if (safeWordCount(value) > 200) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 409) {
        setFieldErrors((prev) => ({
          ...prev,
          general: payload?.message || "You have already applied with this cohort",
        }));
        setSubmitting(false);
        return;
      }
      ...
      if (!res.ok) {
        setFieldErrors((prev) => ({
          ...prev,
          general: payload?.message || "Submission failed. Please try again.",
        }));
        setSubmitting(false);
        return;
      }
  ...
  return (
    <div
      ...
      className="w-full max-w-full px-4 py-4 bg-white rounded-none sm:rounded-lg sm:max-w-md sm:mx-auto max-h-[85vh] overflow-y-auto overflow-x-hidden"
    >
      {fieldErrors.general ? (
