...
export default function FullProgramForm({ onClose }) {
  const sortedCountries = useMemo(() => {
    const base = countries.filter((c) => c.name !== "Other");
  ...
  useEffect(() => {
    try {
      const el = document.getElementById("full-program-form-scroll");
      if (el) el.scrollTop = 0;
    } catch {}
  }, []);

  const monthlyFee = MODE_MONTHLY_KES[form.mode] ?? 0;
  const totalFee = monthlyFee ? monthlyFee * MONTHS : 0;
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            disabled={!isOther}
            required
            className={`w-full sm:w-28 px-3 py-2 rounded border ${
              isOther ? "" : "bg-gray-100"
            }`}
            placeholder="Code"
          />
        ...
        {fieldErrors.phone ? (
          <div className="mt-[-8px] text-sm text-red-700">{fieldErrors.phone}</div>
        ) : null}
        ...
        <div className="pt-2">
          ...
          <input
            type="text"
            value={totalFee ? formatKES(totalFee) : ""}
            disabled
